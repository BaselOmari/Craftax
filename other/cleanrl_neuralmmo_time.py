...
for env_exp in range(10, 11):
    ...
    for iteration in range(TOTAL_TIMESTEPS // (num_envs * ROLLOUT_LENGTH)):
        ...
        # print(f"Step {(iteration + 1) * num_envs * ROLLOUT_LENGTH} -- Time {step_end_time - step_start_time}s")
        
    end_time = time.time()
    print(f"\t\tEnvironment Workers = {num_envs} -- Time = {end_time - start_time}s")
