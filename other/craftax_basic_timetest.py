...
def make_train(config, env):
    ...
    def train(rng):
        ...
        def _update_step(update_runner_state, unused):
            # jax.debug.callback(lambda: print(f"Start Time: {time.time()}"))
            ...
            # jax.debug.callback(lambda: print(f"End Time: {time.time()}"))
            # jax.debug.print("Duration = {}", end_time - start_time)
...
def single_run(config):
    ...
    jax.debug.print("Number of Steps per Iteration: {}", config["NUM_STEPS"] * config["NUM_ENVS"])

    rngs = jax.random.split(rng, config["NUM_SEEDS"])
    train_vjit = jax.jit(jax.vmap(make_train(config, env)))
    train_traced = train_vjit.trace(rngs)
    print("Finished Tracing")
    train_lowered = train_traced.lower()
    print("Finished Lowering")
    train_compiled = train_lowered.compile()
    print("Finished Compilation")

    start = time.time()
    outs = jax.block_until_ready(train_compiled(rngs))
    end = time.time()
...
if __name__ == "__main__":
    config = {
        ...
        "NUM_STEPS": 64,
        "NUM_MINIBATCHES": 1,
        "UPDATE_EPOCHS": 1,  # <-- renamed from NUM_EPOCHS
        "GRU_HIDDEN_DIM": 1,  # <-- renamed from HIDDEN_SIZE
        "FC_DIM_SIZE": 1,     # <-- inferred from usage in ActorCriticRNN
        "ACTIVATION": "tanh",
