...
def make_train(config, env):
    ...
    def train(rng):
        ...
        logenv = LogWrapper(env)
        wrapped_env = CTRolloutManager(logenv, batch_size=config["NUM_ENVS"])
        ...
        def _update_step(runner_state, unused):
            ...
            train_state, buffer_state, prev_state, rng = runner_state
            ...
            (prev_obs, prev_env_state, prev_dones, prev_hs) = prev_state
            expl_state = (prev_hs, prev_obs, prev_dones, prev_env_state)
            rng, _rng = jax.random.split(rng)
            (new_hs, new_obs, new_dones, new_env_state, _), (timesteps, infos) = jax.lax.scan(
                _step_env,
                (*expl_state, _rng),
                None,
                config["NUM_STEPS"],
            ...
            jax.experimental.io_callback(callback, None, metrics, infos)

            runner_state = (
                train_state, 
                buffer_state,
                (new_obs, new_env_state, new_dones, new_hs),
                rng
            )
        ...
        runner_state = (
            train_state, 
            buffer_state,
            (init_obs, env_state, init_dones, init_hs),
            _rng
        )
...
def single_run(config):
    ...
    wandb.init(
        ...
        name=config.get("RUN_NAME", f"{alg_name}_{env_name}"),
        config=config,
