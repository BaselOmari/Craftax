...
def weighted_vdn(qvals, alpha, axis=0):
    """
    Blend each agent's qval with the team average, then
    collapse the agent dimension.
    
        qvals : jnp.ndarray, shape (n_agents, …)
        alpha : scalar in [0,1]
        axis  : which axis is the agent axis (default 0)
    """
    # team mean (keepdims so it broadcasts back to (n_agents, …))
    mean_q = jnp.mean(qvals, axis=axis, keepdims=True)
    mixed  = (1.0 - alpha) * qvals + alpha * mean_q
    return jnp.sum(mixed, axis=axis)
                ...
                def _learn_phase(carry, minibatch):
                    ...
                    def _compute_targets(last_q, q_vals, reward, done):
                        def _get_target(lambda_returns_and_next_q, rew_q_done):
                            ...
                            next_q = weighted_vdn(next_q, config["QVal_Alpha"], axis=0)
                        ...
                        last_q = weighted_vdn(last_q, config["QVal_Alpha"], axis=0)
                    ...
                    def _loss_fn(params):
                        ...
                        #     -1
                        # )  # (num_steps-1*batch_size,)
                        vdn_chosen_action_qvals = (
                            weighted_vdn(chosen_action_qvals, config["QVal_Alpha"], axis=1)[:-1]
                        ).reshape(
                            -1
                        )
...
def tune(default_config):
    ...
    def wrapped_make_train():
        wandb.init(project=default_config["PROJECT"], name=default_config["RUN_NAME"])
    ...
    sweep_config = {
        ...
        "parameters": {
            "LR": {"values": [0.0002, 0.0003, 0.0004]},
            "NUM_ENVS": {"values": [256, 512]},
            "NUM_STEPS": {"values": [8, 16, 32]},
            "GAMMA": {"values": [0.99, 0.995, 0.999, 0.9995]},
...
config = {
    ...
    "RUN_NAME": "disable_spec-3_agent-reduce_actions-alpha_1_test",
    "ENTITY": "b2alomar-university-of-waterloo",
