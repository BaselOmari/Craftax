...
def in_obs_bounds(locations):
    x_inbounds = jnp.logical_and(
        locations[:, 0] >= 0,
        locations[:, 0] < OBS_DIM[0]
    )
    y_inbounds = jnp.logical_and(
        locations[:, 1] >= 0,
        locations[:, 1] < OBS_DIM[1]
    )
    return jnp.logical_and(x_inbounds, y_inbounds)
