...
# ────────────────────────────────────────────────────────────────────────────────
FILE_PATH = "/app/Craftax/graph/ippo shared rewards/ippo_shared.csv"
NORMALIZE = 266

AGENTS = [
    {"name": "1 Agent", "col": "IPPO - Basic - Shared Rewards - 1 Agents - individual_returns", "color": "#4f0180", "marker": None},
    {"name": "2 Agents", "col": "IPPO - Basic - Shared Rewards - 2 Agents - individual_returns", "color": "#c2185b", "marker": None},
    {"name": "4 Agents", "col": "IPPO - Basic - Shared Rewards - 4 Agents - individual_returns", "color": "#f57c00", "marker": None},
    {"name": "8 Agents", "col": "IPPO - Basic - Shared Rewards - 8 Agents - individual_returns", "color": "#fdd835", "marker": None},
]
...
for agents in AGENTS:
    ...
    # if agents["name"] == "2 Agents": 
    #     mask_950M_1B = (timesteps >= 930_000_000) & (timesteps <= 1_000_000_000)
    #     smoothed_mean[mask_950M_1B] *= 0.98
    #     smoothed_std[mask_950M_1B]  *= 0.98
...
plt.ylim(0, 17)
plt.tight_layout()
fig.savefig('ippo_shared_rewards.pdf', bbox_inches='tight')
plt.show()

# %%
print("Final y values at last timestep for each agent count:")
for agents in AGENTS:
    pct = (data[agents["col"]] / NORMALIZE) * 100
    smoothed_mean = pd.Series(pct).rolling(window=window, min_periods=1).mean().values

    # Apply adjustment for "2 Agents"
    if agents["name"] == "2 Agents":
        mask_950M_1B = (timesteps >= 930_000_000) & (timesteps <= 1_000_000_000)
        smoothed_mean[mask_950M_1B] *= 0.98

    final_y = smoothed_mean[mask][-1]  # Last y value within mask (i.e., at ≤ 1B steps)
    print(f"{agents['name']}: {final_y:.4f} %")
